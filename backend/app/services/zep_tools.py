...
class ConsultResult:
    """
    咨询结果 (Consult)
    某个固定Co-Scientist角色（Generation/Reflection/Ranking/Tournament/Evolution/Proximity/MetaReview）
    对一个问题的回答
    """
    role: str  # 被咨询的角色名
    question: str  # 咨询问题
    ...
    def to_text(self) -> str:
        if not self.success:
            return f"咨询 {self.role} 失败：{self.error}"
        text = f"**咨询角色:** {self.role}\n\n"
...
class ZepToolsService:
    ...
    4. consult_specialist - 咨询固定Co-Scientist角色，获取其在研究循环中的推理视角
...
返回JSON格式的子问题列表。"""
    ...
    def consult_specialist(
        ...
        """
        【ConsultSpecialist - 咨询固定角色】

        咨询研究循环中的某个固定Co-Scientist角色（Generation/Reflection/Ranking/
        Tournament/Evolution/Proximity/MetaReview之一），获取其在本轮研究循环中的
        推理视角。通过IPC向正在运行（或已完成主流程、处于待命模式）的研究循环
        进程发送consult命令，由该角色基于其系统提示词与本轮完整上下文直接回答。

        【重要】需要研究循环环境正在运行（或已完成但未关闭）才能使用此功能！

        Args:
            simulation_id: 研究循环ID
            role: 咨询的角色名（Generation/Reflection/Ranking/Tournament/Evolution/Proximity/MetaReview）
            question: 咨询问题
            timeout: 超时时间（秒）
        ...
        logger.info(t("console.interviewAgentsStart", requirement=question[:50]))

        result = ConsultResult(role=role, question=question)

        try:
            api_result = SimulationRunner.consult_agent(
            ...
            result.response = api_data.get("response", "") if isinstance(api_data, dict) else ""

        except ValueError as e:
            result.success = False
            result.error = f"{str(e)}。研究循环环境可能已关闭。"
        except Exception as e:
